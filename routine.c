...
VOID mb(INT code)
{
	...
	if(code && FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, code, 0, buffer, 1024, NULL))
	{
		MessageBox(_r_hwnd, buffer, NULL, 0);
	}
}
...
VOID _r_cfg_init()
{
	...
	StringCchPrintf(_r_cfg_path, MAX_PATH, L"%ls\\%ls.ini", _r_cfg_path, APP_NAME_SHORT);

	if(!_r_helper_fileexists(_r_cfg_path))
	{
...
BOOL _r_cfg_is_portable()
{
	...
	StringCchPrintf(buffer, MAX_PATH, L"%ls\\%ls.ini", buffer, APP_NAME_SHORT);

	return _r_helper_fileexists(buffer);
}
...
VOID _r_cfg_cancer(BOOL makeportable)
{
	...
	if(makeportable != _r_cfg_is_portable())
	{
		if(makeportable)
		{
			...
			StringCchPrintf(path, MAX_PATH, L"%ls\\%ls.ini", path, APP_NAME_SHORT);
		}
		else
		{
...
BOOL _r_cfg_write(LPCWSTR key, LPCWSTR val)
{
	...
	if(!_r_helper_fileexists(buffer))
	{
		...
		if(connect)
		{
			...
			if(dwStatus == HTTP_STATUS_OK)
			{
				...
				if(InternetReadFile(connect, buffera, MAX_PATH, &count) && count)
				{
					...
					if(_r_helper_versioncompare(APP_VERSION, bufferw) == -1)
					{
			...
			_r_cfg_write(L"CheckUpdatesLast", (DWORD)_r_helper_unixtime64());
		}
	}
...
BOOL _r_updatecheck(BOOL is_periodical)
{
	if(is_periodical)
	{
		if(!_r_cfg_read(L"CheckUpdates", 1) || (_r_helper_unixtime64() - _r_cfg_read(L"CheckUpdatesLast", 0)) <= (86400 * ROUTINE_UPDATE_PERIOD)) // update period
		{
			return FALSE;
		}
	}
...
CString _r_helper_formatsize64(DWORDLONG size)
{
	...
	CString buffer;
	buffer.Format(L"%.2f %ls", size_d, sizes[div]);
...
CString _r_helper_formatdate(LPSYSTEMTIME pst)
{
	...
	GetDateFormat(_r_lcid, DATE_LONGDATE, pst, NULL, buffer1, 100);

	buffer = buffer1;
	buffer.Append(L", ");

	GetTimeFormat(_r_lcid, TIME_FORCE24HOURFORMAT | TIME_NOTIMEMARKER, pst, NULL, buffer1, 100);
				...
				if(SUCCEEDED(service->GetFolder(L"\\", &folder)))
				{
					if(SUCCEEDED(folder->GetTask(ROUTINE_TASKSCHD_NAME, &registered_task)))
					{
						if(checkandrun)
						{
							...
							variant_t ticker = buffer.Trim();

							registered_task->RunEx(ticker, TASK_RUN_AS_SELF, 0, NULL, NULL);

							TASK_STATE state;

							do
							{
								registered_task->get_State(&state);
							}

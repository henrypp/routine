...
{
	_r_str_copy (buffer, buffer_length, L"\0");
}
...
)
{
	...
	value = ReadULongPtrAcquire (&event_object->value);
	...
	event_handle = ReadPointerAcquire (&event_object->event_handle);

	if (!event_handle)
	{
		InitializeObjectAttributes (&oa, NULL, OBJ_EXCLUSIVE, NULL, NULL);

		NtCreateEvent (&event_handle, EVENT_ALL_ACCESS, &oa, NotificationEvent, FALSE);
	...
	if (!(ReadULongPtrAcquire (&event_object->value) & PR_EVENT_SET))
	{
...
)
{
	...
	// The Last pointer should be set by list optimization, allowing us to skip all, if not most of the wait blocks.
...
)
{
	...
	value = ReadLongPtrAcquire (&current_value);
	...
	while (TRUE)
	{
		...
		while (!is_ignoreowned && (value & PR_QUEUED_LOCK_OWNED))
		{
			...
			new_value = (LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, (PVOID)new_value, (PVOID)value);
		...
		// Finish up any needed optimization (setting the Previous pointers) while finding the last wait block.
		wait_block = _r_queuedlock_getwaitblock (value);
		...
		{
			// We have an exclusive waiter and there are multiple waiters. We'll only be waking this waiter.
			...
			if (!is_ignoreowned)
				InterlockedExchangeAddPointer ((volatile PLONG_PTR)&queued_lock->value, -PR_QUEUED_LOCK_TRAVERSING); // Clear the traversing bit.

			break;
		}
		else
		{
			// We're waking an exclusive waiter and there is only one waiter, or we are waking a shared waiter and possibly others.
			new_value = (LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, NULL, (PVOID)value);
...
FORCEINLINE BOOLEAN _r_queuedlock_pushwaitblock (
	...
	_Out_ PLONG_PTR new_value_ptr,
	_Out_ PLONG_PTR current_value_ptr
)
{
	LONG_PTR new_value;
...
)
{
	...
	if (!_interlockedbittestandreset (&wait_block->flags, PR_QUEUED_WAITER_SPINNING_SHIFT))
	{
...
HANDLE _r_queuedlock_getevent ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		status = NtCreateKeyedEvent (&hevent, KEYEDEVENT_ALL_ACCESS, &oa, 0);
...
)
{
	...
	LONG_PTR current_value;
	LONG_PTR new_value;
	LONG_PTR value;
	BOOLEAN is_optimize;

	value = ReadLongPtrAcquire (&queued_lock->value);

	while (TRUE)
	{
		if (!(value & PR_QUEUED_LOCK_OWNED))
		{
			new_value = (LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, (PVOID)(value + PR_QUEUED_LOCK_OWNED), (PVOID)value);
		...
		else
		{
			...
			{
				if (is_optimize)
					_r_queuedlock_optimizelist (queued_lock, current_value, FALSE);
...
)
{
	...
	while (TRUE)
	{
		...
		if ((value & (PR_QUEUED_LOCK_WAITERS | PR_QUEUED_LOCK_TRAVERSING)) != PR_QUEUED_LOCK_WAITERS)
		{
			...
			new_value = (LONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, (PVOID)new_value, (PVOID)value);
...
)
{
	...
	if (value & PR_QUEUED_LOCK_MULTIPLE_SHARED)
	{
		...
		if (_InterlockedDecrement (&wait_lock->shared_owners) > 0)
			return;
	}
...
)
{
	...
	current_value = ReadLongPtrAcquire (&value);

	while (TRUE)
	{
		assert (current_value & PR_QUEUED_LOCK_TRAVERSING);
		...
		new_value = (LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, (PVOID)new_value, (PVOID)current_value);
...
)
{
	...
	current_value = (LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, (PVOID)new_value, (PVOID)value);
...
)
{
	...
	value = ReadLongPtrAcquire (&condition->value);
	...
	while (TRUE)
	{
		...
		{
			if (is_optimize)
				_r_queuedlock_optimizelist (condition, current_value, TRUE);
...
)
{
	...
	while (TRUE)
	{
		value = ReadLongPtrAcquire (&protection->value);

		if (value & PR_RUNDOWN_ACTIVE)
			return FALSE;

		new_value = (LONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&protection->value, (PVOID)(value + PR_RUNDOWN_REF_INC), (PVOID)value);
...
)
{
	...
	while (TRUE)
	{
		...
		if (value & PR_RUNDOWN_ACTIVE)
		{
			// Since rundown is active, the reference count has been moved to the waiter's wait block. If we are the last user, we must wake up the waiter.
			wait_block = (PR_RUNDOWN_WAIT_BLOCK)(value & ~PR_RUNDOWN_ACTIVE);

			if (InterlockedDecrementPointer ((volatile PLONG_PTR)&wait_block->count) == 0)
...
)
{
	...
	value = (LONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&protection->value, LongToPtr (PR_RUNDOWN_ACTIVE), NULL);
	...
	while (TRUE)
	{
		...
		new_value = (LONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&protection->value, (PVOID)((LONG_PTR)&wait_block | PR_RUNDOWN_ACTIVE), (PVOID)value);
...
)
{
	...
	if (!hsemaphore)
	{
		...
		NtCreateSemaphore (&hsemaphore, SEMAPHORE_ALL_ACCESS, &oa, 0, MAXLONG);
...
)
{
	...
	while (TRUE)
	{
		...
		else
		{
			...
			{
				_InterlockedDecrement (&work_queue->current_threads);
...
HANDLE NTAPI _r_mem_getheap ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		if (heap_handle)
		{
			mode = HEAP_COMPATIBILITY_MODE_LFH;

			RtlSetHeapInformation (heap_handle, HeapCompatibilityInformation, &mode, sizeof (ULONG));
		}
...
)
{
	...
	old_count = _InterlockedExchangeAdd (&object_header->ref_count, -1);
...
)
{
	...
	_r_obj_movereference ((PVOID_PTR)&sb->string, new_string);
}
...
)
{
	...
	dst = _r_obj_getarrayitem (array_node, index);
	src = _r_obj_getarrayitem (array_node, index + 1);

	if (array_node->cleanup_callback)
	{
		for (ULONG_PTR i = index; i < (index + 1); i++)
		{
	...
	RtlMoveMemory (dst, src, (array_node->count - index - 1) * array_node->item_size);
...
)
{
	...
	RtlMoveMemory (&list_node->items[index], &list_node->items[index + 1], (list_node->count - index - 1) * sizeof (PVOID));
...
)
{
	...
	if (hashtable_entry->hash_code && hashtable_entry->hash_code != ULONG_MAX)
	{
...
BOOLEAN _r_obj_enumhashtable (
	...
	_Out_opt_ PULONG out_hash_code_ptr,
...
)
{
	...
	while (*enum_key < hashtable->next_entry)
	{
		...
		if (hashtable_entry->hash_code != ULONG_MAX)
		{
			if (entry_ptr)
				*entry_ptr = &hashtable_entry->body;

			if (out_hash_code_ptr)
				*out_hash_code_ptr = hashtable_entry->hash_code;
...
)
{
	...
	for (ULONG_PTR i = hashtable->buckets[index]; i != SIZE_MAX; i = hashtable_entry->next)
	{
		...
		if (_r_obj_validatehash (hashtable_entry->hash_code) == valid_hash)
		{
			...
			hashtable_entry->hash_code = ULONG_MAX; // indicates the entry is not being used
...
{
	return _r_obj_createhashtable (sizeof (R_OBJECT_POINTER), initial_capacity, &_r_util_cleanhashtablepointer_callback);
}
...
)
{
	...
	is_success = _r_obj_enumhashtable (hashtable, (PVOID_PTR)&object_ptr, out_hash_code_ptr, enum_key);
...
)
{
	...
	while (remaining_part.length != 0)
	{
		...
		if (directory_part.length != 0)
		{
			if (_r_obj_isstringempty (directory_path))
			{
				_r_obj_movereference ((PVOID_PTR)&directory_path, _r_obj_createstring2 (&directory_part));
			}
			else
			{
				...
				_r_obj_movereference ((PVOID_PTR)&directory_path, directory_name);
			}
		}
	}
...
)
{
	...
	status = _r_fs_dospathnametontpathname (dst_path, &nt_path);

	if (!NT_SUCCESS (status))
		return status;

	status = _r_fs_openfile (src_path, FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN_REPARSE_POINT, FALSE, &hfile);

	if (NT_SUCCESS (status))
	{
		length = (sizeof (FILE_LINK_INFORMATION) + nt_path.Length);

		pfli = _r_mem_allocate (length);

		RtlCopyMemory (pfli->FileName, nt_path.Buffer, nt_path.Length);

		pfli->FileNameLength = nt_path.Length;
		pfli->ReplaceIfExists = TRUE;

		status = NtSetInformationFile (hfile, &isb, pfli, length, FileLinkInformation);
...
)
{
	...
	if (_r_sys_isosversiongreaterorequal (WINDOWS_10_RS5))
	{
		...
		status = NtSetInformationFile (hfile, &isb, &fdi_ex, sizeof (FILE_DISPOSITION_INFO_EX), FileDispositionInformationEx);
	}
	else
	{
		status = STATUS_UNSUCCESSFUL;
	...
	if (!NT_SUCCESS (status))
	{
		...
		status = NtSetInformationFile (hfile, &isb, &fdi, sizeof (FILE_DISPOSITION_INFORMATION), FileDispositionInformation);
	}
...
)
{
	...
	// vista+
	status = CoCreateInstance (&CLSID_FileOperation, NULL, CLSCTX_INPROC_SERVER, &IID_IFileOperation, (PVOID_PTR)&file_ops);
	...
	status = SHCreateItemFromParsingName (sr->buffer, NULL, &IID_IShellItem, (PVOID_PTR)&shell_item);
	...
	while (TRUE)
	{
		...
		while (TRUE)
		{
			...
			if (directory_info->FileNameLength && !_r_str_isempty2 (directory_info->FileName))
			{
				...
				if (!_r_str_isequal2 (&file_name->sr, L".", FALSE) && !_r_str_isequal2 (&file_name->sr, L"..", FALSE))
				{
					...
					if (!enum_callback (file_name, path_full, directory_info->FileAttributes, &directory_info->CreationTime, &directory_info->LastWriteTime, context))
						is_break = TRUE;
...
)
{
	...
	status = NtQueryVolumeInformationFile (hfile, &isb, &info, sizeof (FILE_FS_SIZE_INFORMATION), FileFsSizeInformation);
...
)
{
	...
	status = NtQueryInformationFile (hfile, &isb, &info, sizeof (FILE_POSITION_INFORMATION), FilePositionInformation);
...
)
{
	...
	status = NtQueryInformationFile (hfile, &isb, &info, sizeof (FILE_STANDARD_INFORMATION), FileStandardInformation);
...
)
{
	...
	status = NtQueryInformationFile (hfile, &isb, &info, sizeof (FILE_BASIC_INFORMATION), FileBasicInformation);
...
{
	MOUNTMGR_MOUNT_POINT mpp = {0};
	PMOUNTMGR_MOUNT_POINTS buffer;
	IO_STATUS_BLOCK isb;
	ULONG in_length = sizeof (MOUNTMGR_MOUNT_POINT);
	ULONG out_length;
	ULONG attempts = 16;
	NTSTATUS status;

	out_length = 0x800;
	buffer = _r_mem_allocate (out_length);

	do
	{
		status = NtDeviceIoControlFile (hdrive, NULL, NULL, NULL, &isb, IOCTL_MOUNTMGR_QUERY_POINTS, &mpp, in_length, buffer, out_length);
		...
		if (status == STATUS_BUFFER_OVERFLOW)
		{
			...
			if (out_length > PR_SIZE_BUFFER_OVERFLOW)
			{
				*out_buffer = NULL;
...
)
{
	...
	if (NT_SUCCESS (status) && (attributes & FILE_ATTRIBUTE_DIRECTORY) == FILE_ATTRIBUTE_DIRECTORY)
		return TRUE;
...
)
{
	...
	status = NtSetInformationFile (hfile, &isb, &info, sizeof (FILE_BASIC_INFORMATION), FileBasicInformation);
...
)
{
	...
	return RtlSetCurrentDirectory_U (&us);
}
...
)
{
	...
	return NtSetInformationFile (hfile, &isb, &info, sizeof (FILE_BASIC_INFORMATION), FileEndOfFileInformation);
}
...
)
{
	...
	return NtSetInformationFile (hfile, &isb, &info, sizeof (FILE_BASIC_INFORMATION), FileBasicInformation);
}
...
)
{
	...
	PR_STRING full_path;
	PR_STRINGREF sr;
	ULONG buffer_length = 0x0400;
	ULONG return_length;
	NTSTATUS status;

	if (_r_str_isnullterminated (filename))
	{
		sr = filename;
	}
	else
	{
		string = _r_obj_createstring2 (filename);
	...
	status = RtlGetFullPathName_UEx (sr->buffer, buffer_length, full_path->buffer, NULL, &return_length);
	...
	if (NT_SUCCESS (status))
	{
		if (return_length > buffer_length)
		{
			...
			_r_obj_movereference ((PVOID_PTR)&full_path, _r_obj_createstring_ex (NULL, buffer_length * sizeof (WCHAR)));
...
)
{
	...
	if (search_path)
	{
		_r_obj_initializeunicodestring2 (&search_path_us, search_path);
	}
	else
	{
		...
		_r_obj_initializeunicodestring2 (&search_path_us, &string->sr);
	...
	status = RtlDosSearchPath_Ustr (flags, &search_path_us, &filename_us, &extension_us, NULL, NULL, NULL, NULL, &return_length);
	...
	if (status == STATUS_SUCCESS || status == STATUS_BUFFER_TOO_SMALL)
	{
		...
		status = RtlDosSearchPath_Ustr (flags, &search_path_us, &filename_us, &extension_us, &buffer, NULL, NULL, NULL, NULL);
	}
...
)
{
	...
	{
		if (hwnd)
			_r_show_errormessage (hwnd, L"Could not write to key!", status, NULL, ET_NATIVE);
...
)
{
	...
	*out_path = NULL;
	...
	status = CoCreateInstance (&CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, &IID_IShellLinkW, (PVOID_PTR)&shell_link);

	if (SUCCEEDED (status))
	{
		status = IShellLinkW_QueryInterface (shell_link, &IID_IPersistFile, (PVOID_PTR)&persist_file);

		if (SUCCEEDED (status))
		{
			string = _r_obj_createstring2 (lnk_path);

			status = IPersistFile_Load (persist_file, string->buffer, STGM_READ);
...
)
{
	...
	if (!_r_fs_exists (path))
	{
		if (_r_path_getpathinfo (path, &directory_sr, NULL))
		{
			path = &directory_sr;
		}
		else
...
)
{
	...
	if (status == STATUS_BUFFER_TOO_SMALL)
	{
		_r_obj_movereference ((PVOID_PTR)&buffer_string, _r_obj_createstring_ex (NULL, buffer_length));
...
)
{
	...
	{
		_r_obj_movereference ((PVOID_PTR)&buffer, _r_obj_createstring2 (path));
	}
...
)
{
	...
	if (!is_ignorecase)
	{
		if (HAS_INTRINSICS)
		{
			...
			if (length16 != 0)
			{
				pattern = _r_intrinsics_setint128by16 (character);

				do
				{
					block = _r_intrinsics_loadint128u ((PLONG)buffer);
					block = _r_intrinsics_compareint128by16 (block, pattern);
					mask = _r_intrinsics_movemaskint128by8 (block);
...
)
{
	...
	{
		unixtime = _r_str_tolong64 (string);
...
)
{
	...
	return _r_str_gethash (&sr, is_ignorecase);
}
...
)
{
	...
	if (HAS_INTRINSICS)
	{
		p = (LPWSTR)((ULONG_PTR)string & ~0xE); // string should be 2 byte aligned
...
)
{
	...
	while (remaining_part.length != 0)
	{
		...
		if (_r_str_splitatchar (&values_part, value_delimeter, &key_part, &value_part))
		{
			...
			hash_code = _r_str_gethash (&key_part, TRUE);
...
)
{
	...
	if (is_ignorecase)
	{
		for (LPWSTR buffer = string->buffer; buffer != end_buffer; buffer++)
		{
			...
			hash_code = ((65599 * hash_code) + (ULONG)chr);
		}
	}
	else
	{
...
)
{
	...
	if (NT_SUCCESS (status))
	{
		...
		{
			_r_obj_initializebyteref_ex (&bytes, (LPSTR)entry->Text, entry->Length);
...
PR_STRINGREF _r_sys_getcommandline ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		string = _r_obj_createstring3 (&NtCurrentPeb ()->ProcessParameters->CommandLine);
...
PR_TOKEN_ATTRIBUTES _r_sys_getcurrenttoken ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		status = _r_sys_querytokeninformation (token_handle, TokenUser, (PVOID_PTR)&token_user);
...
PR_STRINGREF _r_sys_getimagepath ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		string = _r_obj_createstring3 (&NtCurrentPeb ()->ProcessParameters->ImagePathName);
...
)
{
	...
	_r_obj_initializebyteref (&br, (LPSTR)modules->Modules[0].FullPathName);
...
)
{
	while (TRUE)
	{
		out_buffer->HighPart = USER_SHARED_DATA->SystemTime.High1Time;
		out_buffer->LowPart = USER_SHARED_DATA->SystemTime.LowPart;

		if (out_buffer->HighPart == USER_SHARED_DATA->SystemTime.High2Time)
			break;
...
ULONG _r_sys_getwindowsversion ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		if (NT_SUCCESS (status))
		{
			...
			{
				if (version_info.dwBuildNumber >= 26220) // 25h2-dev
				{
					windows_version = WINDOWS_NEW;
				}
				else if (version_info.dwBuildNumber >= 26200)
				{
					windows_version = WINDOWS_11_25H2;
				}
...
)
{
	...
	if (file_name)
	{
		...
		if (!_r_fs_exists (&file_name_string->sr))
		{
			...
			if (NT_SUCCESS (status))
			{
				_r_obj_movereference ((PVOID_PTR)&file_name_string, new_path);
			}
			else
			{
				_r_obj_clearreference ((PVOID_PTR)&file_name_string);
			}
		}
	}
	...
	if (NT_SUCCESS (status) && is_wait)
	{
		...
		if (NT_SUCCESS (NtCreateJobObject (&hjob, JOB_OBJECT_ALL_ACCESS, &oa)))
		{
			...
			do
			{
				if (!NT_SUCCESS (NtQueryInformationJobObject (hjob, JobObjectBasicAccountingInformation, &basic_info, sizeof (JOBOBJECT_BASIC_ACCOUNTING_INFORMATION), NULL)))
					break;
...
)
{
	...
	status = RtlCompressBuffer (format, (PUCHAR)buffer->buffer, (ULONG)buffer->length, (PUCHAR)tmp_buffer->buffer, allocation_length, 4096, &return_length, ws_buffer);
...
)
{
	...
	status = RtlDecompressBuffer (format, (PUCHAR)tmp_buffer->buffer, allocation_length, (PUCHAR)buffer->buffer, (ULONG)buffer->length, &return_length);
	...
	if (status == STATUS_BAD_COMPRESSION_BUFFER)
	{
		do
		{
			...
			_r_obj_movereference ((PVOID_PTR)&tmp_buffer, _r_obj_createbyte_ex (NULL, allocation_length));
...
)
{
	...
	switch (error_code)
	{
		...
		{
			return STATUS_ILL_FORMED_SERVICE_ENTRY; // STATUS_INVALID_CONFIG_VALUE
		}
...
)
{
	...
	data.ProcessId = ULongToHandle (process_id);
...
)
{
	...
	PrivateExtractIconsW (string->buffer, icon_id, icon_size, icon_size, &hicon, NULL, 1, LR_DEFAULTCOLOR);
...
)
{
	...
	{
		name_hash = _r_str_gethash2 (icon_name, TRUE);
	}
...
)
{
	...
	client_id.UniqueThread = ULongToHandle (thread_id);
...
)
{
	...
	if (is_register)
	{
		status = RegisterApplicationRestart (_r_sys_getcommandline ()->buffer, RESTART_NO_CRASH | RESTART_NO_HANG | RESTART_NO_PATCH);
	}
	else
	{
...
)
{
	...
	tag_query.InParams.ProcessId = process_id;
	tag_query.InParams.ServiceTag = PtrToUlong (tag);

	_I_QueryTagInformation (NULL, TagInfoLevelNameFromTag, &tag_query);

	if (tag_query.OutParams.Name)
	{
		service_name_string = _r_obj_createstring (tag_query.OutParams.Name);

		LocalFree ((HLOCAL)tag_query.OutParams.Name);
...
#if defined(_WIN64)
	time_value.QuadPart = *(volatile PULONG64)&USER_SHARED_DATA->SystemTime;
#else
	do
	{
...
)
{
	...
	paint_params.cbSize = sizeof (BP_PAINTPARAMS);
...
)
{
	...
	if (htheme)
	{
		...
		// vista+
		is_success = SUCCEEDED (DrawThemeTextEx (htheme, hdc, part_id, state_id, string->buffer, (INT)_r_str_getlength2 (string), flags, rect, &dtto));
	}
	else
	{
		if (clr_text)
		...
		is_success = (DrawTextExW (hdc, string->buffer, (ULONG)_r_str_getlength2 (string), rect, flags, NULL) != 0);
...
)
{
	...
	return IFileDialog_Show (file_dialog->ifd, hwnd);
}
...
)
{
	...
	if (_r_path_getpathinfo (path, &directory_part, &basename_part))
	{
		string = _r_obj_createstring2 (&directory_part);

		status = SHParseDisplayName (string->buffer, NULL, &item, 0, &attributes);
	...
	else
	{
		...
		IFileDialog_SetFileName (file_dialog->ifd, string->buffer);
...
)
{
	...
	_r_obj_addlistitem (layout_manager->list, layout_item, NULL);
...
)
{
	...
	if (hparent)
	{
		...
		_r_wnd_adjustrectangletoworkingarea (hwnd, &rectangle);
...
)
{
	...
	if (!GetClientRect (hwnd, &rect))
	{
		RtlZeroMemory (rectangle, sizeof (R_RECTANGLE));
...
)
{
	...
	while (TRUE)
	{
		...
		if (IsDialogMessageW (hmain_wnd, &msg))
			is_processed = TRUE;
...
)
{
	...
	{
		is_success = (ShowWindow (hwnd, is_minimized ? SW_RESTORE : SW_SHOW) != 0);
...
{
	return _r_math_hashinteger_ptr ((LONG_PTR)hwnd) ^ property_id;
}
...
)
{
	...
	if (_r_sys_isosversiongreaterorequal (WINDOWS_8_1))
	{
		...
		_r_inet_setoption (hsession, WINHTTP_OPTION_DECOMPRESSION, WINHTTP_DECOMPRESSION_FLAG_GZIP | WINHTTP_DECOMPRESSION_FLAG_DEFLATE);
	}
	else
	{
	...
	_r_inet_setoption (hsession, WINHTTP_OPTION_SECURE_PROTOCOLS, protocols);

	// disable redirect from https to http
	_r_inet_setoption (hsession, WINHTTP_OPTION_REDIRECT_POLICY, WINHTTP_OPTION_REDIRECT_POLICY_DISALLOW_HTTPS_TO_HTTP);

	// enable http2 protocol (win10+)
	if (_r_sys_isosversiongreaterorequal (WINDOWS_10))
		_r_inet_setoption (hsession, WINHTTP_OPTION_ENABLE_HTTP_PROTOCOL, WINHTTP_PROTOCOL_FLAG_HTTP2);

	// disable global, cross-session pooling (win11+)
	if (_r_sys_isosversiongreaterorequal (WINDOWS_11))
	{
		_r_inet_setoption (hsession, WINHTTP_OPTION_DISABLE_GLOBAL_POOLING, TRUE);

		_r_inet_setoption (hsession, WINHTTP_OPTION_TLS_FALSE_START, TRUE);
	}
...
)
{
	...
	*hrequest_ptr = NULL;
	...
	_r_inet_setoption (hsession, WINHTTP_OPTION_DISABLE_FEATURE, WINHTTP_DISABLE_KEEP_ALIVE);
	...
	do
	{
		...
		if (!is_valid)
		{
			...
			else if (status == ERROR_WINHTTP_CONNECTION_ERROR)
			{
				...
				is_valid = _r_inet_setoption (hsession, WINHTTP_OPTION_SECURE_PROTOCOLS, WINHTTP_FLAG_SECURE_PROTOCOL_TLS1_2);
			...
			{
				is_valid = _r_inet_setoption (hsession, WINHTTP_OPTION_SECURITY_FLAGS, SECURITY_FLAG_IGNORE_UNKNOWN_CA | SECURITY_FLAG_IGNORE_CERT_WRONG_USAGE);
...
)
{
	...
	return NtDeleteValueKey (hkey, &us);
}
...
)
{
	...
	if (status == STATUS_BUFFER_TOO_SMALL)
	{
		_r_obj_movereference ((PVOID_PTR)&string, _r_obj_createstring_ex (NULL, buffer_length - sizeof (UNICODE_NULL)));
	...
	if (NT_SUCCESS (status))
	{
		if (type == REG_EXPAND_SZ)
		{
			...
			if (NT_SUCCESS (status))
				_r_obj_movereference ((PVOID_PTR)&string, expanded_string);
		}
...
)
{
	...
	if (path->buffer[0] == L'@')
	{
		...
		path_string = _r_path_find (&path_part, FALSE);
...
)
{
	...
	{
		_r_str_printf (entry, RTL_NUMBER_OF (entry), L"\\StringFileInfo\\%08X\\%s", lcid_arr[i], entry_name);
	...
	return _r_obj_createstring_ex (buffer, length);
...
)
{
	...
	*out_length = 0;
	...
	is_success = (_VerQueryValue (block, sub_block, out_buffer, out_length) != 0);
...
)
{
	...
	ver_info = (GetFileVersionInfoExW (FILE_VER_GET_LOCALISED, path, 0, ver_size, ver_block) != 0);
...
{
	IImageList2* himagelist = NULL;
	COLORREF clr_prev;
	HRESULT status;

	status = ImageList_CoCreateInstance (&CLSID_ImageList, NULL, &IID_IImageList2, &himagelist);
	...
	status = IImageList2_Initialize (himagelist, width, height, flags, initial_count, grow_count);

	if (SUCCEEDED (status))
	{
		IImageList2_SetBkColor (himagelist, CLR_NONE, &clr_prev);

		*out_buffer = IImageListToHIMAGELIST (himagelist);
	}
	else
	{
		*out_buffer = NULL;
...
)
{
	...
	*out_buffer = hashtable;
	...
	while (!_r_str_isempty (sections_iterator.buffer))
	{
		...
		{
			if (section_list)
				_r_obj_addlistitem (section_list, _r_obj_createstring2 (&sections_iterator), NULL);
			...
			while (!_r_str_isempty (values_iterator.buffer))
			{
				...
				if (*values_iterator.buffer != L'#')
				{
					if (_r_str_splitatchar (&values_iterator, L'=', &key_string, &value_string))
					{
						...
						{
							if (!_r_obj_findhashtable (hashtable, hash_code))
								_r_obj_addhashtablepointer (hashtable, hash_code, value_string.length ? _r_obj_createstring2 (&value_string) : NULL);
						}
...
)
{
	...
	is_success = (Shell_NotifyIconW (NIM_ADD, &nid) != 0);
...
)
{
	...
	is_success = _r_tray_popup (hwnd, guid, flags, title, string->buffer);
...
)
{
	...
	is_success = _r_tray_setinfo (hwnd, guid, hicon, string->buffer);
...
)
{
	...
	is_succeess = (_r_wnd_sendmessage (hwnd, ctrl_id, BCM_SETTEXTMARGIN, 0, (LPARAM)&rect) != 0);
...
)
{
	...
	tool_info.cbSize = sizeof (TTTOOLINFOW);
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, EM_SHOWBALLOONTIP, 0, (LPARAM)&ebt) != 0);
}
...
)
{
	...
	is_success = _r_ctrl_showballoontip (hwnd, ctrl_id, icon_id, title, string->buffer);
...
)
{
	...
	return (InsertMenuItemW (hmenu, MAXUINT, TRUE, &mii) != 0);
}
...
)
{
	...
	return (InsertMenuItemW (hmenu, pos, TRUE, &mii) != 0);
}
...
#pragma warning(disable: 26454)
	...
	result = _r_wnd_sendmessage (hwnd, ctrl_id, TCM_SETCURSEL, (WPARAM)item_id, 0);
...
)
{
	...
	while (count >= allocated_count)
	{
		...
		_r_obj_movereference ((PVOID_PTR)&string, _r_obj_createstring_ex (NULL, allocated_count * sizeof (WCHAR)));
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETCOLUMN, (WPARAM)column_id, (LPARAM)&lvc) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETITEM, 0, (LPARAM)&lvi) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETITEMSTATE, (WPARAM)item_id, (LPARAM)&lvi) != 0);
}
...
)
{
	...
	if (ex_style)
		_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, ex_style);
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TVM_SETITEM, 0, (LPARAM)&tvi) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, RB_INSERTBAND, (WPARAM)INT_ERROR, (LPARAM)&rbi) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_INSERTBUTTON, (WPARAM)button_id, (LPARAM)&tbi) != 0);
}
...
)
{
	...
	if (_r_str_isstartswith (&string->sr, app_name, FALSE))
	{
		// check window prop
